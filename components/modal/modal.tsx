  ...
  return (
    <div className={active ? 'visible opacity-100 ' : 'invisible opacity-0 '}>
      <div
        className={
          active
            ? 'fixed bg-black/70 top-0 right-0 left-0 bottom-0 scale-100 z-10 backdrop-blur-sm'
            : 'fixed bg-black/70 top-0 right-0 left-0 bottom-0 scale-0 z-10'
        }
        onClick={onClose}
      />
      <div
        className={
          active
            ? 'w-[1024px] p-5 rounded bg-white absolute top-1/2 right-1/2 translate-x-1/2 -translate-y-1/2 shadow-lg shadow-black z-10 visible opacity-100 transition-all'
